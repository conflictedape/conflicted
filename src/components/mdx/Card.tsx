...
export function Card({ title, icon, span = 1, children }: CardProps) {
	return (
		...
		>
			...
			<div className="[&_a]:text-primary flex-1 px-4 py-4 text-sm leading-6 [&_a]:no-underline [&_a:hover]:underline [&_ol]:m-0 [&_p]:m-0 [&_p]:mb-2 [&_p]:font-medium [&_ul]:m-0 [&_ul]:list-none [&_ul]:space-y-1 [&_ul]:pl-4 [&_ul_li]:relative [&_ul_li]:pl-3 [&_ul_li]:before:absolute [&_ul_li]:before:top-2 [&_ul_li]:before:left-0 [&_ul_li]:before:h-1 [&_ul_li]:before:w-1 [&_ul_li]:before:rounded-full [&_ul_li]:before:bg-current [&_ul_li]:before:opacity-60">
				{children}
			</div>
		</div>
	);
