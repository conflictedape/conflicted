...
function InlineCard({ id, title }: { id: string; title: string }) {
	...
	return (
		...
		>
			<div className="relative shrink-0 overflow-hidden rounded-lg" style={{ width: 120, aspectRatio: '16/9' }}>
				<img
					src={thumbnail}
					alt={title}
					loading="lazy"
...
export function Youtube({ id, title, caption, variant = 'embed', width = 'md', start }: YoutubeProps) {
	if (variant === 'inline') {
